...
/*   Updated: 2024/04/07 12:00:58 by sentry           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_echo(t_msh *msh, t_cmd *cmd)
{
	...
	set_last_exit_status(msh, 0); // ADDED 06.04.24 - Set zero exit status for success
}
...
{
    bool print_newline;
    char exit_status_str[12]; // Enough space for a 32-bit integer
    ...
    t_list *curr_arg = cmd->arguments;
    while (curr_arg)
    {
        char *arg = (char *)curr_arg->data;
        if (strcmp(arg, "$?") == 0)
        {
            sprintf(exit_status_str, "%d", msh->last_exit_status);
            ft_printf_fd(cmd->out_fd, "%s", exit_status_str);
        }
        else
        {
            ft_printf_fd(cmd->out_fd, "%s", arg);
        }
