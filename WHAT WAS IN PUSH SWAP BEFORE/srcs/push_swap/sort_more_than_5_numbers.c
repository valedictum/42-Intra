...
/*   Updated: 2023/11/12 16:58:19 by atang            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
                    ...
                    int *operation_count)
{
    ...
    while (*stack_b != NULL && target_rank <= max_rank)
    {
        ...
        // Determine the correct position for the element in stack A
        while (*stack_a != NULL && (*stack_a)->rank < target_element->rank)
        {
            ra(stack_a, operation_count);
        }

        // Push the element to stack A at the correct position
        pa(stack_a, stack_b, operation_count);

        // Update positions and move to the next target rank
        ps_update_positions(*stack_a);
        ps_update_positions(*stack_b);
        target_rank++;
    }

    // Rotate stack A to ensure it is sorted
    while (!ps_check_if_sorted(*stack_a))
    {
        ra(stack_a, operation_count);
    }
