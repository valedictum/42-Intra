...
/*   Updated: 2023/12/15 01:22:58 by sentry           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
is set up to handle signalling behaviour i.e. the action to be taken when a signal 
is received. A welcome message, process ID (pid), and 'waiting' message are printed. 
The 'sa_sigaction' member of the 'sa' structure is set to 'handle_signal' which will 
be called when a signal is received. 'sigemptyset(&sa.sa_mask)' sets the sa_mask 
member to an empty set. 'sa_flags' is set to SA_SIGINFO indicating that additional 
information about the signal will be passed to the signal handler. The 'sigaction' 
function is then called to set up the signal handlers for SIGUSR1 and SIGUSR2. 
The 'pause' function is then called to pause the program execution until a signal 
is received.
*/
