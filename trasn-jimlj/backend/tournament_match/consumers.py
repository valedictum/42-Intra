...
class PongConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if not PongConsumer.rooms_state[self.room_name]["game_started"]:
            print(f"TOURNEY MATCH STARTED: {self.room_name}", flush=True)
    ...
    async def disconnect(self, close_code):
        ...
        if username == PongConsumer.rooms_state[self.room_name]["p1_expected"]:
            ...
            print(f"A fixture player has disconnected: {username} ", flush=True)
    ...
    async def game_loop(self):
        ...
        # # EXPERIMENTAL
        # print(f"match index: {room_state["match_index"]}", flush=True)
        # print(f"round index: {room_state["round_index"]}", flush=True)
    ...
    #     PongConsumer.rooms_state[self.room_name]["game_state"] = new_state
    ...
    async def set_gamestate(self, new_state: GameState):
        ...
        print(f"Game state changed from {old_state.name} to {new_state.name} in room {self.room_name}", flush=True)
