...
/*   Updated: 2024/04/26 22:43:38 by sentry           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void    write_status_debug(t_philo_status status, t_philo *philo, long   elapsed)
{
    ...
    if(TAKE_SECOND_FORK == status && !sim_finished(philo->data))
...
void    write_status(t_philo_status status, t_philo *philo, bool debug)
{
    ...
    elapsed = get_time(MILLISECOND) - philo->data->start_sim; // - start sim is NEW
    ...
    else
    {
        ...
        //else if (FULL == status)
        //    printf(W"%-6ld"RST" all philosophers have eaten [%ld] times\n",
        //    elapsed, philo->meals_count);
    }
    // Unlock
