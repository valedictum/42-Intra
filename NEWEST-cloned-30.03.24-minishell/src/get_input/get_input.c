...
char	*get_input(t_msh *msh, char *prompt)
{
	...
	if (!line)
	{
		//if (isatty(STDIN_FILENO)) // (CTRL-D) DELETE this line
		//	error("readline error"); // (CTRL-D) DELETE this line	
		printf("exit\n"); // (CTRL-D) ADD this line
		free_everything(msh);
	...
	*/
	//(CTRL-C) DELETE the above commented out block...
	//(CTRL-C) and ADD the below
	//(CTRL-C) FROM HERE...
	if (*line)
		add_history(line);
	else
