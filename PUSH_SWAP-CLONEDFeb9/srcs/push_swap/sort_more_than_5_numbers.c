			...
			int *operation_count)
{
	...
	while (i < 2)
	{
		...
		while (current != NULL && !is_sorted(*stack_a))
		{
			if (current->rank >= (i * (get_max_rank(*stack_a) / 2) + 1)
				&& current->rank <= ((i + 1) * (get_max_rank(*stack_a) / 2)))
				add_to_chunk(&chunk_head, &chunk_tail, current);
			current = current->next;
		}
		if (chunk_head != NULL)
...
void chunk_sort_2(t_stack_node **stack_a, t_stack_node **stack_b,
    ...
    while (i < 2) {
        ...
        set_chunk_null(&chunk_head, &chunk_tail);
        while (current != NULL && !is_sorted(*stack_a)) {
            int lower_limit = i * (max_rank / 2) + 1;
            int upper_limit = (i + 1) * (max_rank / 2) + (max_rank % 2 / 2);

            if (current->rank >= lower_limit && current->rank <= upper_limit) {
                add_to_chunk(&chunk_head, &chunk_tail, current);
            }
            current = current->next;
        }
			...
			int *operation_count)
{
	...
	while (i < 10)
	{
		current = *stack_a;
		set_chunk_null(&chunk_head, &chunk_tail);
		while (current != NULL && !ps_check_if_sorted_descending(*stack_a))
		{
			if (current->rank >= (i * (get_max_rank(*stack_a) / 10) + 1)
				&& current->rank <= ((i + 1) * (get_max_rank(*stack_a)
						/ 10)))
				add_to_chunk(&chunk_head, &chunk_tail, current);
			current = current->next;
		}
