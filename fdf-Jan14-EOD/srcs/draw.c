...
/*   Updated: 2024/01/14 18:23:28 by atang            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	bresenham(t_point start, t_point end, t_fdf *data)
{
	...
	while ((int)(x - x1) || (int)(y - y1))
	{
	    img_x = (int)x;
        img_y = (int)y;
        if (img_x >= 0 && img_x < 1920 && img_y >= 0 && img_y < 1080)
        {
            img_index = img_y * (1920) + img_x;
...
/*
void apply_transformations(float *x, float *y, int z, t_fdf *data)
{
    *x = (*x * data->zoom + data->shift_x) * data->amplify;
    *y = (*y * data->zoom + data->shift_y) * data->amplify;
    isometric(x, y, z * data->amplify);
}
...
void draw(t_fdf *data)
{
    ...
    while (y < data->height)
    {
        ...
        while (x < data->width)
        {
            if (x < data->width - 1)
            {
            	apply_transformations(&x, &y, data->z_matrix[y][x], data);
				bresenham((t_point){x, y, data->z_matrix[y][x]},
					(t_point){x + 1, y, data->z_matrix[y][x + 1]}, data);
            }
            if (y < data->height - 1)
