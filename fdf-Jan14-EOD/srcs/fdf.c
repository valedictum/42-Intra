...
/*   Updated: 2024/01/14 18:27:44 by atang            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv)
{
	...
	if (file_exists(argv[1]))
	{
		...
		//OR the below instead
		//mlx_clear_window(data->mlx_ptr, data->win_ptr);
		//draw (data);
		//mlx_clear_window(data->mlx_ptr, data->win_ptr);
		data->offset_x = ((1920 - data->width) * data->zoom) / 2;
		data->offset_y = ((1080 - data->height) * data->zoom) / 2;
		// Aded the next line
		mlx_put_image_to_window(data->mlx_ptr, data->win_ptr, data->img_ptr, data->offset_x, data->offset_y);
